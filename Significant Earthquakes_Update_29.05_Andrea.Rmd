...
#!!!Updates by Andrea!!!: - this section can be deleted after updates.
 - Table of contents - copy the whole everywhere updates
 - chunk imports
 - all inputs in:
  2.2 [Plotting continous variables](#Plotting continous variables)  
  2.3 [Plotting categorical variables](#Plotting categorical variables)
...
eqdata.gam <- eqdata %>%
  select(Deaths,Death.Description, Mag, MMI.Int, Focal.Depth..km., Longitude, Latitude, Region, Country, Freq.Country,Freq.Country )%>%
  filter(!is.na(MMI.Int)) %>%
  filter(!is.na(Focal.Depth..km.))%>%
  filter(Freq.Country > 3)

str(eqdata.gam)
unique(eqdata.gam$Country)

set.seed(155)

# Making Splits in country groups
splits = group_split(eqdata.gam, group_by = Country)
...
sum(is.na(Valid$Deaths))
sum(is.na(Valid$Death.Description))
sum(is.na(Valid$Mag))
sum(is.na(Valid$MMI.Int))
sum(is.na(Valid$Focal.Depth..km.))
sum(is.na(Valid$Longitude))
sum(is.na(Valid$Latitude))
sum(is.na(Valid$Region))
sum(is.na(Valid$Country))

#Checking Data sets if split was successful
collectTest$Country
collectTrain$Country
str(collectTrain)
str(collectTest)
str(collectRemaining)
str(Remaining.train)
str(Remaining.test)

str(Train)
str(Valid)
unique(Train$MMI.Int)
unique(Valid$MMI.Int)
unique(Train$Country)
unique(Valid$Country)


#write.table(eqdata.gam, file="eqdata.tsv", quote=TRUE,  sep = '\t')
#write.table(collectTrain, file="Train.tsv", quote=TRUE,  sep = '\t')
#write.table(collectTest, file="Valid.tsv", quote=TRUE,  sep = '\t')
...
The summary output in both models below indicates that there there is a strong evidence that Magnitude has linear effects on the response variable. 
There is no evidence, that the values of Intensity2, Latitude or Focal Depth an  influence on the number of death caused by an earthquake. 

The models explain above 50% the overall variability, based on the R squared value. 

There is no evidence that a region would have a strong effect a number of death number levels. 
We can observe that few of the country levels seem to have a medium or minor effect on the increasing of the death number levels. This countries are those where the reported earthquakes were more likely to have a high Magnitude, High frequency and a high value of intensity level.

<br>

<button type="button" class="collapsible">**Results of Fitting the GAM 1: Response Variable Deaths + Country (interactive drop down button)**</button>
<div class="content">
  <p>
...
# Fitting GAM1 with Country

gam.death1 <- gam(Death.Description ~ Mag + factor(MMI.Int) + Focal.Depth..km. + Country + s(Longitude) + Latitude, data = Train, familiy = multinom(K=4), na.action=na.exclude)

summary(gam.death1)

par(mfrow=c(1,4))
gam.check(gam.death1)
...
```{r fitting GAM1.1.1 death , eval=TRUE, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, include=TRUE}

par(mfrow=c(2,3))
vis.gam(gam.death1, view = c("Longitude","Mag"))
vis.gam(gam.death1, view = c("Longitude","Latitude"))
vis.gam(gam.death1, view = c("Longitude", "Focal.Depth..km."))
vis.gam(gam.death1, view = c("Focal.Depth..km.","Mag"))
#vis.gam(gam.death1, view = c("Focal.Depth..km.","Longitude"))
vis.gam(gam.death1, view = c("Focal.Depth..km.","Latitude"))
#vis.gam(gam.death1, view = c("Mag","Focal.Depth..km."))
#vis.gam(gam.death1, view = c("Mag","Longitude"))
vis.gam(gam.death1, view = c("Mag","Latitude"))
#vis.gam(gam.death1, view = c("Latitude","Focal.Depth..km."))
#vis.gam(gam.death1, view = c("Latitude","Longitude"))
#vis.gam(gam.death1, view = c("Latitude","Mag"))
```

<br>

<button type="button" class="collapsible">**Results of Fitting the GAM 2: Response Variable Deaths + Region (interactive drop down button)**</button>
<div class="content">
  <p>

```{r fitting GAM1.1.2 death , eval=TRUE, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, include=TRUE}
# Fitting GAM1 with Region

gam.death2 <- gam(Death.Description ~ Mag + factor(MMI.Int) + s(Focal.Depth..km.) + factor(Region) + s(Longitude) + s(Latitude), data = Train, familiy = multinom(K=4),na.action=na.exclude)
...
```{r fitting GAM1.2.1 death , eval=TRUE, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, include=TRUE}
...
These models with the count death numbers as response variable seem to have a higher value by explaining the variability of both models. (higher than 90 %)
However the summary outputs indicate in both cases that there there is strong evidence that almost all regions and most of the country levels as well as some of the intensity levels may have a strong effect on the outcome variable. 
...
<button type="button" class="collapsible">**Results of Fitting the GAM 3: Response Variable Deaths + Country (interactive drop down button)**</button>
<div class="content">
  <p>

```{r fitting GAM2.1 deaths , eval=TRUE, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, include=TRUE}

# Fitting GAM2 with Country

gam.deaths1 <- gam(Deaths  ~ s(Mag) + factor(MMI.Int) +s(Focal.Depth..km.) + s(Longitude) + s(Latitude) + Country, data =Train, family = "quasipoisson", na.action=na.exclude)

summary(gam.deaths1)

par(mfrow=c(1,4))
gam.check(gam.deaths1, title ="GAM Death Counts with Country")
...
```{r fitting GAM2.1.2 deaths , eval=TRUE, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, include=TRUE}

par(mfrow=c(2,3))
vis.gam(gam.deaths1, view = c("Longitude","Mag"))
vis.gam(gam.deaths1, view = c("Longitude","Latitude"))
vis.gam(gam.deaths1, view = c("Longitude", "Focal.Depth..km."))
vis.gam(gam.deaths1, view = c("Focal.Depth..km.","Mag"))
#vis.gam(gam.deaths1, view = c("Focal.Depth..km.","Longitude"))
vis.gam(gam.deaths1, view = c("Focal.Depth..km.","Latitude"))
#vis.gam(gam.deaths1, view = c("Mag","Focal.Depth..km."))
#vis.gam(gam.deaths1, view = c("Mag","Longitude"))
vis.gam(gam.deaths1, view = c("Mag","Latitude"))
#vis.gam(gam.deaths1, view = c("Latitude","Focal.Depth..km."))
#vis.gam(gam.deaths1, view = c("Latitude","Longitude"))
#vis.gam(gam.deaths1, view = c("Latitude","Mag"))

```

<br>

<button type="button" class="collapsible">**Results of Fitting the GAM 4: Response Variable Deaths + Region (interactive drop down button)**</button>
<div class="content">
  <p>
...
```{r fitting GAM2.2.2 deaths , eval=TRUE, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, include=TRUE}
...
#### 6.4. Model Interpretation and Crossvalidation <a name="Model Interpretation and Evaluation"></a>
...
# Cross validation models with Country - categorical response
print("1. GAM Model: Death.Description response variable / Country:")
predicted.country1 <- predict(gam.death1, newdata= Valid)
r.squared.gam.country1 <- cor(predicted.country1,Valid$Deaths,use = "complete.obs")^2
mean(r.squared.gam.country1)

# Cross validation models with Region - categorical response
print("2. GAM Model: Death.Description response variable / Region:")
predicted.reg1  <- predict(gam.death2, newdata= Valid)
r.squared.gam.reg1 <- cor(predicted.reg1, Valid$Deaths, use = "complete.obs")^2
mean(r.squared.gam.reg1)

# Cross validation models with Country - count response
print("3. GAM Model: Deaths response variable / Country:")
predicted.country2  <- predict(gam.deaths1, newdata= Valid)
r.squared.gam.country2  <- cor(predicted.country2 ,Valid$Deaths, use="complete.obs")^2
mean(r.squared.gam.country2 )

# Cross validation models with Region - count response
print("4. GAM Model: Deaths response variable / Region:")
predicted.reg2  <- predict(gam.deaths2, newdata= Valid)
r.squared.gam.reg2 <- cor(predicted.reg2,Valid$Deaths, use = "complete.obs")^2
mean(r.squared.gam.reg2)
